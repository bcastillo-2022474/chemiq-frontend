      ...
      <Card className="h-64 overflow-hidden bg-white hover:shadow-md transition-all duration-300">
        <div className="flex h-full">
          <div className="w-1/3 relative">
            <img src={article.img || "/placeholder.svg"} alt={article.titulo} className="object-cover w-full h-full"/>
          ...
          <CardContent className="w-2/3 p-4 flex flex-col justify-between">
            <div>
              <div className="flex items-center mb-2">
                ...
                <h3 className="text-xl font-semibold text-[#0B2F33] truncate">{article.titulo}</h3>
              </div>
              <p className="text-sm text-[#0B2F33]/70 line-clamp-2">{article.contenido}</p>
            </div>
            <div className="flex justify-between items-center mt-4">
              <div className="flex items-center">
                <Calendar className="h-4 w-4 text-[#28BC98] mr-1"/>
                <span className="text-xs text-[#0B2F33]/70">{formatDate(article.created_at)}</span>
              </div>
              <div
