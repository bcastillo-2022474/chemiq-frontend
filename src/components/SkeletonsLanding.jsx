...
function Skeleton({ className, ...props }) {
  return <div className={cn("animate-pulse rounded-md bg-tertiary/70", className)} {...props} />
}
      ...
      <div className="w-full h-[400px] bg-gradient-to-r from-tertiary via-background to-tertiary rounded-lg relative overflow-hidden">
        ...
        <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2">
          {[1, 2, 3, 4, 5].map((_, index) => (
            <div key={index} className={`h-2 w-2 rounded-full bg-subase/50 ${index === 0 ? "w-6 bg-base/70" : ""}`} />
          ))}
        </div>
